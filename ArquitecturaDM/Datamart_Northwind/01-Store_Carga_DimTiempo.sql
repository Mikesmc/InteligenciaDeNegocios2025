...
BEGIN
  ...
  IF @AddUnknown = 1
  BEGIN
    IF NOT EXISTS (SELECT 1 FROM dim_date WHERE date_key = 19000101)
    BEGIN
      INSERT dim_date(date_key, [Date], [Day], [Month], MONTH_NAME, [Quarter], [Year], week_of_year, is_weekend)
      VALUES (
        19000101,
        '1900-01-01',
        1,
  ...
  (
    ...
    WHERE x.date_key = CONVERT(INT, FORMAT(d.[Date], 'yyyyMMdd'))
  )
